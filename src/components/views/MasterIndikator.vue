...
export default {
  ...
  methods: {
    newRow() {
      axios.post('http://10.199.14.46:8005/api/MasterIndikator/', this.form).then(res => {
        console.log(res.data)
      })
    },
    updateRow(instance, cell, columns, row, value) {
      axios.get('http://10.199.14.46:8005/api/MasterIndikator/').then(res => {
        var index = Object.values(res.data[row])
        index[columns] = value
        console.log(index)
        axios.put('http://10.199.14.46:8005/api/MasterIndikator/' + index[0], {
          id: index[0],
          id_aspek: index[1],
          id_penyebut: index[2],
    ...
    deleteRow(instance, row) {
      axios.get('http://10.199.14.46:8005/api/MasterIndikator/').then(res => {
        ...
        // console.log(index)
        console.log(row)
        axios.delete('http://10.199.14.46:8005/api/MasterIndikator/' + index[0])
      })
    }
  },
  computed: {
    jexcelOptions() {
      return {
        ...
        allowToolbar: true,
        url: 'http://10.199.14.46:8005/api/MasterIndikator/',
